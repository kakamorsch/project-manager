...
const ProjectList = () => {
  ...
  const ProjectDeleteButton = ({ projectId }) => {
    ...
    }, [deleteProject]); // Removido refreshProjects das dependências
    ...
    return (
      <button
        ...
        className={`${
          deleteLoading ? 'bg-gray-400' : 'bg-red-500 hover:bg-red-700'
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {loading ? (
        ...
        <div className="mt-4 text-red-500 text-center">Error loading projects: {error.message}</div>
      ) : projects && projects.length > 0 ? (
        <Table projects={projects} ProjectDeleteButton={ProjectDeleteButton} />
      ) : (
...
const Table = ({ projects, ProjectDeleteButton }) => (
